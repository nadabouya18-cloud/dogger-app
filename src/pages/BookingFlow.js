...
const getSavedWalker = () => {
 const w = localStorage.getItem('dogger_walker');
 return w ? JSON.parse(w) : null;
};
const getSavedService = () => localStorage.getItem('dogger_walk_service') || 'Balade';
const getSavedDuration = () => parseInt(localStorage.getItem('dogger_walk_active') || '30');
const getSavedAddress = () => localStorage.getItem('dogger_walk_address') || '';
const getSavedCoords = () => {
 const c = localStorage.getItem('dogger_user_coords');
...
export default function BookingFlow() {
 ...
 // Si une balade est déjà en cours avec un walker trouvé, on repart de là
 ...
 const [form, setForm] = useState({
   ...
   instructions: '', service: 'walk', duration: getSavedDuration(), date: '', time: ''
 });
 ...
 const [userCoords, setUserCoords] = useState(getSavedCoords());
 ...
 const initMap = useCallback(() => {
   ...
   directionsService.route({
     ...
     if (status === 'OK') {
       ...
       setEtaSeconds(prev => prev === getSavedEta() ? duration : prev);
       localStorage.setItem('dogger_walker_eta', String(duration));
     }
   });
 }, [userCoords]);
 ...
 useEffect(() => {
   ...
   const interval = setInterval(() => {
     setRouteIndex(i => {
       const next = i + 1;
       if (next >= totalSteps) { clearInterval(interval); return totalSteps - 1; }
       if (walkerMarkerRef.current) walkerMarkerRef.current.setPosition(routePath[next]);
       return next;
     });
   }, Math.max(msPerStep, 800));
   return () => clearInterval(interval);
 }, [matched, routePath, walkerPhase]);

 // Countdown ETA — toujours actif quand matched et pas arrivé
 useEffect(() => {
   ...
   const interval = setInterval(() => {
     setEtaSeconds(s => {
       ...
       if (next <= 120 && next > 0) {
         setWalkerPhase(p => {
           if (p !== 'arriving' && p !== 'here') {
             localStorage.setItem('dogger_walker_phase', 'arriving');
             return 'arriving';
           }
           return p;
         });
       ...
       if (next <= 0) {
         setWalkerPhase('here');
         localStorage.setItem('dogger_walker_phase', 'here');
 ...
 useEffect(() => {
   ...
   const interval = setInterval(() => {
     ...
     if (i >= SEARCH_STEPS.length) {
       clearInterval(interval);
       setTimeout(() => {
         const w = { name: 'Thomas M.', rating: 4.9, walks: 127, emoji: '🧑' };
   ...
   return (
     <div style={{ minHeight: '100vh', background: '#fff', fontFamily: 'sans-serif', maxWidth: 430, margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
       ...
       <div style={{ position: 'relative', height: 320 }}>
         ...
         {/* Badge si pas encore de carte chargée */}
         {!userCoords && (
