    ...
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", maxWidth: 430, margin: '0 auto' }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, #0F6E56 0%, #1D9E75 100%)', padding: '60px 24px 48px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button onClick={() => navigate('/login?redirect=walker')} style={{ flex: 1, padding: '14px 20px', background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1.5px solid rgba(255,255,255,0.35)', borderRadius: 14, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
            Promener
          </button>
        </div>
      </div>
      ...
      {/* WALKER BANNER */}
      <div onClick={() => navigate('/register-walker')} style={{ margin: '20px 20px 0', background: 'linear-gradient(135deg, #0F6E56, #0A4D3A)', borderRadius: 16, padding: '20px', display: 'flex', alignItems: 'center', gap: 16, cursor: 'pointer' }}>
