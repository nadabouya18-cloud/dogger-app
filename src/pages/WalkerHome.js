...
export default function WalkerHome() {
  ...
  const [phase, setPhase] = useState('idle'); // idle | mission_incoming | mission_accepted | navigating | walking | finished
  ...
  // Timer mission (30s pour accepter)
  useEffect(() => {
  ...
  // Init carte navigation
  const initNavMap = useCallback(() => {
    if (!mapRef.current || !window.google || mapInstanceRef.current) return;
    // Position simulée Paris
  ...
  useEffect(() => {
    if ((phase === 'navigating' || phase === 'walking') && mapRef.current) {
    ...
    <div style={{ minHeight: '100vh', background: '#F8FAF9', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", maxWidth: 430, margin: '0 auto', paddingBottom: 80 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: '28px 24px 40px', width: '100%', maxWidth: 430, animation: 'slidein 0.3s ease' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: '#888' }}>Nouvelle mission !</div>
            </div>

            {/* Infos mission */}
            <div style={{ background: '#F8FAF9', borderRadius: 16, padding: '16px', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#E1F5EE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
              ...
              <div style={{ fontSize: 13, color: '#1D9E75', fontWeight: 600 }}>🚶 À {mission.distance} de vous</div>
              {mission.instructions && (
                <div style={{ fontSize: 12, color: '#888', marginTop: 8, background: '#FFF8E1', borderRadius: 8, padding: '6px 10px' }}>
                  💬 {mission.instructions}
                </div>
              )}
            </div>
      ...
      {showRating && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: '28px 24px 40px', width: '100%', maxWidth: 430 }}>
            ...
            <div style={{ background: '#E1F5EE', borderRadius: 12, padding: '12px 16px', marginBottom: 20, textAlign: 'center' }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: '#1D9E75' }}>+{mission?.price}€</div>
      ...
      <div style={{ background: 'linear-gradient(160deg, #0F6E56, #1D9E75)', padding: '48px 24px 24px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div onClick={() => { setAvailable(a => !a); if (!available) setPhase('idle'); }}
            style={{ width: 52, height: 28, borderRadius: 14, background: available ? '#fff' : 'rgba(255,255,255,0.3)', cursor: 'pointer', position: 'relative', transition: 'background 0.3s' }}>
          ...
          <div style={{ animation: 'slidein 0.3s ease' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
              {[
                ...
                { label: 'Ce mois', value: '243€', icon: '📅' },
                { label: 'Balades', value: history.length + 127, icon: '🐾' },
              ].map(s => (
            ...
            {!mission || phase === 'idle' ? (
              ...
              <div>
                ...
                <div style={{ position: 'relative', marginBottom: 16 }}>
                  <div ref={mapRef} style={{ height: 240, borderRadius: 18, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.1)' }} />
                ...
                {/* Photos pendant la balade */}
                {phase === 'walking' && (
                  <div style={{ background: '#fff', borderRadius: 16, padding: '16px', marginBottom: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#1A1A1A', marginBottom: 12 }}>📸 Envoyer des photos</div>
                    ...
                    {photos.length > 0 && (
                      <div style={{ fontSize: 12, color: '#1D9E75', fontWeight: 600 }}>✅ {photos.length} photo{photos.length > 1 ? 's' : ''} envoyée{photos.length > 1 ? 's' : ''} au propriétaire</div>
                    )}
                  </div>
                )}
          ...
          <div style={{ animation: 'slidein 0.3s ease' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 16, padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              {[
                ...
                <div key={item.label}
                  style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: '1px solid #F0F0F0', cursor: 'pointer' }}>
